...
const Auth = () => {
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'azure') => {
    try {
      ...
      if (error) {
        toast.error(`Feil ved innlogging med ${provider === 'google' ? 'Google' : 'Microsoft'}`);
      }
    } catch (error: any) {
            ...
            <Button type="submit" size="lg" className="w-full" disabled={isSubmitting}>
              {isSubmitting ? (
                <>
                  ...
                  {isForgotPassword ? 'Sender e-post...' : isSignUp ? 'Registrerer...' : 'Logger inn...'}
                </>
              ) : (
                isForgotPassword ? 'Send tilbakestillings-e-post' : isSignUp ? 'Registrer deg' : 'Logg inn'
              )}
            </Button>
          </form>
          ...
          <div className="mt-4 text-center text-sm space-y-2">
            ...
            {!isSignUp && (
              <button
                ...
                className="text-muted-foreground hover:text-primary hover:underline text-xs block w-full"
                disabled={isSubmitting}
              >
                {isForgotPassword ? 'Tilbake til innlogging' : 'Glemt passordet?'}
              </button>
            )}
          </div>
